...
A valid improvement must introduce, remove, clarify, constrain, prioritize, or restructure at least one instruction, distinction, decision rule, or reasoning requirement in a way that directly addresses the evaluation evidence.

If the available evidence does not justify a large change, make the smallest substantive change supported by the evidence rather than producing a stylistic paraphrase.
...
2. Use the Recommendations as the primary guidance for improvement, while validating them against the supporting evaluation evidence.

3. Prioritize systematic weaknesses and recurring problems over isolated failures.

4. Use the Aggregated Metrics as optimization signals. Give greater attention to weaker dimensions while avoiding changes likely to degrade stronger dimensions.

5. Use Passed Evaluations to identify behavior that should be preserved. Do not optimize failed examples at the expense of behavior that already performs well.

6. Use Failed Examples to understand concrete manifestations of the identified weaknesses. Pay particular attention to their plausible causes, missing conceptual distinctions, general rules, and regression risks.

7. Prefer changes supported by multiple failed examples or by the broader Recommendations over rules that only explain a single unusual example.

8. Treat each \`generalRule\` as a proposed generalization, not an instruction that must be copied into the prompt. Determine whether it is supported by the broader evidence and express it in the improved prompt only when it is likely to generalize.

9. Use \`regressionRisks\` to constrain improvements. When introducing or strengthening a rule, ensure it does not become so broad that it harms behaviors known to succeed.

10. Do not infer fixed semantics from the keys of the original example data. Rely on the accompanying evaluation analysis to understand what the example demonstrates.

11. Do not add instructions that mention, encode, quote, or otherwise specialize the prompt to particular evaluation examples.

12. Make the smallest effective set of substantive changes. Prefer targeted modifications over rewriting the prompt from scratch unless the analysis provides strong evidence of a fundamental design problem.

13. Every modification should have an operational purpose. It should alter how the prompt guides interpretation, reasoning, prioritization, classification, decision-making, constraint handling, or output generation.

14. Do not treat stylistic rewriting as optimization. Changes that only improve wording, readability, organization, conciseness, or tone are insufficient unless they also resolve a behaviorally relevant ambiguity or weakness identified by the evaluation evidence.

15. When an existing instruction is already adequate, preserve it rather than rewriting it merely to make the candidate appear different.

16. Prefer precise, actionable instructions over vague guidance. Resolve ambiguity or conflicts where supported by the analysis.

17. Keep the improved prompt clear, concise, internally consistent, and free of unnecessary duplication.

18. Preserve the Original Prompt's intent, input/output contract, constraints, and expected behavior unless the evaluation analysis provides strong evidence that one of them should change.

19. Do not include commentary, explanations, analysis, or a changelog in the improved prompt unless the Original Prompt itself requires them.
